...
sed -i 's/address = "localhost:9000"/address = "0.0.0.0:9000"/' /opt/listbox/config.toml
sed -i 's/^password = ".*"/password = "'"$DB_PASS"'"/' /opt/listbox/config.toml
$STD /opt/listmonk/listmonk --install --config /opt/listmonk/config.toml
